...
contract AgroTechFarmCrowdsale is Ownable {    
  ...
  function enableRefunds() onlyOwner public {
    require(address(this).balance < softcap && state == State.Active  && _getTime() > end);
  ...
  function refund() public  {
   ...
   function createTokens() public saleIsOn payable {
     ...
     uint bonusTokens = 0;
     if(_getTime() <= start.add(10 days)) {
       bonusTokens = tokens.mul(20).div(100);
     } else if(_getTime() > start.add(10 days) && _getTime() <= start.add(25 days)) {
       bonusTokens = tokens.mul(10).div(100);
     } else if(_getTime() > start.add(25 days) && _getTime() < start.add(40 days)) {
